package com.android.spin.draw.view;
...
public class LuckPanLayout extends View {
    ...
    private void drawSmallCircle(boolean FirstYellow){
        int pointDistance = radius - DensityUtil.dp2px(10);
        for(int i=0;i<=360;i+=20){
            int x = (int) (pointDistance * Math.sin(DensityUtil.change(i))) + CircleX;
            int y = (int) (pointDistance * Math.cos(DensityUtil.change(i))) + CircleY;

            if(FirstYellow)
                canvas.drawCircle(x,y,DensityUtil.dp2px(4),yellowPaint);
            else
                canvas.drawCircle(x,y,DensityUtil.dp2px(4),whitePaint);
